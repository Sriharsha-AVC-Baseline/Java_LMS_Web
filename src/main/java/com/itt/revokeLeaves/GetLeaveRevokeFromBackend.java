...
public class GetLeaveRevokeFromBackend {
	...
	{
		String statement,decision;
		Connect connect = Connect.getConnection();
		PreparedStatement execStatement;
		if(iAmTheManager==true)
		{
		 statement = "select *from leave_records where emp_id=(select ex.exec_id from execu ex,manager m where  m.manager_id=ex.manager_id and m.manager_id=?) and status=? or status=?";
		  execStatement = connect.connection.prepareStatement(statement);
			execStatement.setString(1, pass);
			execStatement.setString(2, LeaveStatusEnum.APPROVED.toString());
			execStatement.setString(3, LeaveStatusEnum.APPROVED_BY_LEAD.toString());
		}
		else
		{
			 statement = "select *from leave_records where emp_id=(select ex.exec_id from execu ex,leads l where l.lead_id=ex.lead_id and l.lead_id=?) and status=?";
			  execStatement = connect.connection.prepareStatement(statement);
				execStatement.setString(1, pass);
				execStatement.setString(2, LeaveStatusEnum.APPROVED_BY_LEAD.toString());
		}
	...
	public List<com.itt.check_leaves.LeaveRecords> getLeaveRecordsOfLeads(String pass) throws SQLException, ClassNotFoundException
	{
		...
		leadStatement.setString(2, LeaveStatusEnum.APPROVED.toString());
