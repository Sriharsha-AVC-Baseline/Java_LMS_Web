...
public class DecideRevokeBackend {
	public void decideLeave(String empId,String leaveType,String leaveStatus,String fromDate,String toDate,String decision) throws ClassNotFoundException, SQLException
	{
		if(decision.equalsIgnoreCase("later"))
		{
			...
			String state = "update leave_records set status=? where EMP_ID=? and leave_type=? and FROM_DATE=? and to_date=? and status=? or status=?;";
			...
			approveStatement.setString(6, com.itt.message_constants.LeaveStatusEnum.APPROVED.toString());
			approveStatement.setString(7, com.itt.message_constants.LeaveStatusEnum.APPROVED_BY_LEAD.toString());
			approveStatement.executeUpdate();
			
			if(decision == com.itt.message_constants.LeaveStatusEnum.REVOKED.toString())
			{
				addLeave(empId,leaveType,fromDate,toDate);
		}
	}
	
	public void addLeave(String empId,String leaveType,String fromDate,String toDate) throws ClassNotFoundException, SQLException
	{
